...
In this dataset, there are no categorical variables. 
...
### Don't know how to do: make new indicator variables that are 1 if they have a value of `NULL` and 0 otherwise. 

```{r recipe}
hotels_recipeTest <- recipe(is_canceled ~ ., #short-cut, . = all other vars
  ...
  step_mutate(children = ifelse(children > 1, 1, 0), 
              ...
              agent = ifelse(is.null(agent), 1,0), 
              company = ifelse(is.null(company), 1,0), 
...
```{r}
hotel_recipe_TEST2 <- recipe(is_canceled ~ .,
                       data = hotels_training) %>% 
...
## Google
- Logistic regression does not shrink the variables. 

- With LASSO, we only use the most important variables. 
...
- Computer has to define it. 

### In here, should we use linear regression or logistic regression? 
...
In here, the reserved_room_type_P comes up as the most important features. 
...
## Make sure to calculate the true positive rate!
...
## Is true positive in here means: the room we predict to get canceled DID get canceled?
### And true negative: the room we predict not get canceled DID NOT get canceled. 
...
For the model, according to the importance variables graph, ....
...
I don't know which questions to ask. 
